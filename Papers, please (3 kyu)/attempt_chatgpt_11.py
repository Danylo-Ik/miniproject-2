...
class Inspector:
    ...
    def check_info_mismatch(self, entrant):
        if "ID_card" in entrant.keys():
            id_card = entrant["ID_card"]
            id_number = id_card.split("ID#:")[1].split()[0].strip()
            for doc, details in entrant.items():
                if doc != "ID_card":
                    if "ID#:" in details:
                        if id_number != details.split("ID#:")[1].split()[0].strip():
                            return "ID number"
        return None
...
print(inspector.inspect(josef))  # Output: 'Glory to Arstotzka.'
print(inspector.inspect(guyovich))  # Output: 'Entry denied: missing required passport.'
print(inspector.inspect(roman))  # Output: 'Detainment: ID number mismatch.'
