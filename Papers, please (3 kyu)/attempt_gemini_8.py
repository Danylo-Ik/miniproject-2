...
class Inspector:
  ...
  def inspect(self, entrant):
    ...
    # Check for document mismatches (excluding passport for Roman's case)
    for doc1, info1 in entrant.items():
      if doc1 != "passport":  # Skip passport check for Roman
        for doc2, info2 in entrant.items():
          if doc1 != doc2 and info1.split()[1] != info2.split()[1]:
            return f"Detainment: {doc1} {info1.split()[0]} mismatch."
